...
import { CATEGORY_ORDER, selectDailyOutfitGarments, canGenerateOutfit, validateOutfit, validateManualSave, validateSelection, translateOutfitError } from '../src/utils/outfit-generator-v2';
...
export default function CreateOutfitScreen() {
  ...
  useEffect(() => {
    ...
    logger.debug('[Outfit] Initial selection built:', Object.keys(init));
    ...
    setSelectedById((prev) => {
      ...
      for (const [cat, id] of Object.entries(cleaned)) {
        if (id && !validIds.has(id)) {
          logger.debug('[Outfit] Cleaning orphan reference:', cat, '=', id);
          delete cleaned[cat];
          changed = true;
        }
      }
  ...
  const handleAutoGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (!preCheck.allowed) {
        logger.debug('[Outfit] Pre-generation validation failed:', preCheck.reason);
      ...
      logger.debug('[Outfit] Generating with categories:', {
      ...
      if (lastModeRef.current === 'dress') {
        ...
        logger.debug('[Outfit] Last was dress → trying top-bottom');
      } else if (lastModeRef.current === 'top-bottom') {
        preferredMode = 'dress';
        logger.debug('[Outfit] Last was top-bottom → trying dress');
      }

      // STEP 3: Generate the combination
      let result = selectDailyOutfitGarments(allGarments, recentIds, null, excludeIds, preferredMode);
      logger.debug('[Outfit] Generated result:', result.garments.map((g) => ({ id: g.garment_id, cat: g.category_used })), '| mode:', result.outfitMode);

      // STEP 4: Post-generation validation — ensure the result is complete and valid
      let postCheck = validateOutfit(result);
      if (!postCheck.valid) {
        logger.debug('[Outfit] First attempt failed:', postCheck.errors);
      ...
      logger.debug('[Outfit] Valid outfit selected:', newSelection, '| mode:', result.outfitMode);
      lastModeRef.current = result.outfitMode;
      setSelectedById(newSelection);
      setHasAutoGenerated(true);
    } catch (err) {
      logger.warn('[Outfit] Auto-generate error:', err.message);
  ...
  const handleSave = useCallback(async () => {
    if (savingRef.current) {
      logger.debug('[OUTFIT] Already saving — preventing duplicate call');
      return;
    }

    const list = buildSelectionList();
    logger.debug('[OUTFIT] handleSave — selected garments:', list.length, list.map((g) => g.id));

    const validation = validateManualSave(list);
    if (!validation.valid) {
      logger.debug('[OUTFIT] Validation failed:', validation.errors);
      Alert.alert('Corrige la selección', validation.errors.join('\n'));
      return;
    }
    ...
    try {
      ...
      logger.debug('[OUTFIT] Guardado exitosamente:', result.outfit.id);

      Alert.alert(
        '¡Outfit guardado!',
        `"${result.outfit.name}" se guardó correctamente.`,
        [
          { text: 'Ver en Inventario', onPress: () => navigation.navigate('Inventario') },
          { text: 'Seguir creando', style: 'cancel' },
        ],
      );
    } catch (err) {
      logger.warn('[OUTFIT] Save error:', err.message);
      Alert.alert('Error al guardar', 'No se pudo guardar el outfit. Revisa tu conexión e intenta de nuevo.');
    } finally {
      savingRef.current = false;
      setIsSaving(false);
    }
  }, [buildSelectionList, navigation]);
